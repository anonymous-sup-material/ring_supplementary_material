...
@cache
def _load_sys() -> base.System:
    return base.System.from_xml(Path(__file__).parent.joinpath("xmls/arm.xml"))


@cache
def _get_sys(anchor: str, include_links: tuple[str]):
    sys = _load_sys().morph_system(new_anchor=anchor)
...
class IMTP:
    ...
    def sys(self):
        ...
        sys = _get_sys(self.segments[0], tuple(include_links))
        sys = sys.change_model_name(suffix=f"_{len(self.segments)}Seg")
        return sys

    def sys_noimu(self):
        sys_noimu, imu_attachment = self.sys().make_sys_noimu()
...
def _build_Xy_xs_xsnoimu(
    ...
    data = load_data(trial, resample_to_hz=imtp.hz)
    sys = imtp.sys()
    sys_noimu, imu_attachment = imtp.sys_noimu()
...
def benchmark(
    ...
    X, y, xs, xs_noimu = _build_Xy_xs_xsnoimu(trial, imtp)
