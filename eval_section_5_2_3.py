...
def eval_section_5_2_3(method, method_name) -> list[float]:
    mae = []
    for i in range(2, 5):
        imtp = IMTP(
            [f"seg{i}", f"seg{i + 1}"],
...
methods = [
    baselines.TwoSeg1D("1d_corr", ollson=True),
    baselines.TwoSeg1D("euler_1d", ollson=True),
...
if __name__ == "__main__":
    ...
    for method, method_name in zip(methods, method_names):
        mae = eval_section_5_2_3(method, method_name)
